...
output: 
    html_document:  #Add options for html output 
        ...
        toc_float: #have the table of contents floating on the side
...
Table of the variable, description, and units from A1_Data_Excel Dataset
---------------------------------------------------
Variable|Description|Units
--------|-----------|-----
population_type|Distinguishes between Asexual and Sexual population types|Categorical (A/S)
ovary_width|The area of the ovary of the flower|mm
ovary_height|The height of the ovary of the flowe|mm
flower_number|The total number of flowers on the plant|Counts
...
cc1_data <- subset(cc1_data[1:46,1:8])
...
width <- cc1_data$ovary_width_mm
height <- cc1_data$ovary_height_mm
cc1_data$ovary_area <- width*height
ovary_area<-cc1_data$ovary_area
...
(equation here)
$$OvaryWidthMM \times OvaryHeightMM$$
The units are $mm^2$
...
library(ggplot2)
ggplot(cc1_data, aes(x=ovary_area)) + geom_histogram(binwidth=0.5) + facet_wrap(~population_type) + theme_classic() + xlab("Ovary Area (mm^2)") + ylab("Frequency") + labs(title="Distribution of Ovary Area by Population Type", caption="This histogram compares the Ovary Area in mm^2 between two Population Types, Sexual (S) and Asexual (A))")
...
Source: [Solanum pimpinellifolium](https://onq.queensu.ca/d2l/lms/dropbox/user/folder_submit_files.d2l?db=492096&grpid=1208701&isprv=0&bp=0&ou=1208667)
